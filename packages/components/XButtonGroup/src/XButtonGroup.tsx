...
const XButtonGroup = defineComponent({
    ...
    setup(props, { slots }) {
        ...
        onMounted(() => {
            ...
            xButtonList =
                xButtonGroupRef.value?.querySelectorAll<HTMLButtonElement>(
                    ".x-button"
                );
            if (xButtonList !== undefined && xButtonList.length > 0) {
                ...
                if (props.direction === "horizontal") {
                    changeHorizontalStyle(
                        xButtonList[0],
                        xButtonList[xButtonList.length - 1]
                    );
                    changeHorizontalStyle(
                        xButtonList[0].querySelector(".x-button__click")!,
                        xButtonList[xButtonList.length - 1].querySelector(
                            ".x-button__click"
                        )!
                    );
                }
        ...
        return () => (
            <div
                ...
                ref={xButtonGroupRef}
                style={groupStyle[props.direction]()}
            >
                {slots.default ? slots.default() : <span>放点东西吧~</span>}
            </div>
        );
    },
});
...
function changeHorizontalStyle(
    firstChangeDom: HTMLElement,
    lastChangeDom: HTMLElement
) {
